...
void EditorController::onMouseDrag(const int dx, const int dy, const Qt::MouseButtons buttons)
{
...
void EditorController::onMouseRelease(const Qt::MouseButton button) const
{
    if (canRotateCamera(button) || button == Qt::RightButton) {
        mRenderWidget->releaseMouse();
        mRenderWidget->unsetCursor(); // Restore the normal arrow cursor
    }
}
